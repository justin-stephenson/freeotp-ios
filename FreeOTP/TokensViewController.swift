...
class TokensViewController : UICollectionViewController, UICollectionViewDelegateFlowLayout, UIPopoverPresentationControllerDelegate {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let token = store.load(indexPath.row) {
            ...
            if token.image == nil {
                let faIconName = "fa-" + token.issuer

                if let iconName = FontAwesome(rawValue: faIconName) {
                    cell.image.image = UIImage.fontAwesomeIcon(name: iconName, style: .brands, textColor: .white,
    ...
    @IBAction func iconsClicked(_ sender: UIBarButtonItem) {
        let _: UIViewController = self.next("icons", sender: sender, dir: [.left, .right])
    }
